...
import { Building2, ChevronLeft, Crosshair, FlaskConical, Gem, Hammer, Pickaxe, Rocket, Scale, Shield, ShoppingCart, Trophy, Users, X, Zap } from "lucide-react";
import { ALLIANCE_CREATE_ANTIMATTER_COST, ALLIANCE_CREATE_USDC_COST, BUILDINGS, SHIPS, SHIP_TYPE_IDX, energyEfficiency, upgradeCost, type AllianceJoinRequestAccount, type AllianceMembershipAccount, type AllianceStateAccount, type AllianceTreasuryStateAccount, type GameClient, type PlayerState, type QuestProgressStateAccount, type QuestRewardTargetStateAccount, type QuestStateAccount, type StoreConfigState, type StorePurchaseStateAccount } from "./game-state";
...
export default function PlanetWorld({ state, busy, run, onExit, wallet, game, market, worlds, ownedWorlds, marketControls, vaultControls, allianceBalances }: { state: PlayerState; busy: boolean; run: (label: string, action: (client: GameClient, entity: PublicKey) => Promise<unknown>) => Promise<void>; onExit: () => void; wallet: PlanetWalletSummary | null; game: GameClient | null; market: MarketClient | null; worlds: Array<{ entity: string; name: string }>; ownedWorlds: PlayerState[]; marketControls: { onTxStart: (label: string) => void; onTxEnd: (error?: string) => void }; vaultControls: { depositAmount: string; withdrawAmount: string; onDepositAmountChange: (value: string) => void; onWithdrawAmountChange: (value: string) => void; onDeposit: () => Promise<void>; onWithdraw: () => Promise<void>; busy: boolean }; allianceBalances: { usdc: bigint; antimatter: bigint } }) {
  const [selected, setSelected] = useState(0);
  const [modal, setModal] = useState<"buildingSelect" | "building" | "research" | "ships" | "defense" | "missions" | "quests" | "store" | "alliance" | "market" | null>(null);
  ...
  return <main className="planet-world">
    ...
    <nav className="pw-operations" aria-label="Planet operations"><button title="Buildings" onClick={() => setModal("buildingSelect")}><Building2 /></button><button title="Research" onClick={() => setModal("research")}><FlaskConical /></button><button title="Shipyard" onClick={() => setModal("ships")}><Rocket /></button><button title="Defenses" onClick={() => setModal("defense")}><Shield /></button><button title="Missions" onClick={() => setModal("missions")}><Crosshair /></button><button title="Market" onClick={() => setModal("market")}><Scale /></button><button title="Quests" onClick={() => setModal("quests")}><Trophy /></button><button title="Alliance" onClick={() => setModal("alliance")}><Users /></button><button title="Store" onClick={() => setModal("store")}><ShoppingCart /></button></nav>
